...
class Pi0Config(_model.BaseModelConfig):
    ...
    # Optional explicit future-state bottleneck for pi0.5. When enabled, the model first predicts
    # q_{t+1:t+H}, re-embeds the numerical prediction as a cached state block, and conditions action
    # flow matching on that block.
    predict_future_states: bool = False
    # Number of meaningful state dimensions before padding to action_dim. Explicitly setting this
    # prevents padded dimensions from becoming an unsupervised latent side channel.
    ...
    def __post_init__(self):
        if self.predict_future_states and not self.pi05:
            raise ValueError("Future-state conditioning is currently supported only for pi0.5.")
        if self.state_dim is None:
            object.__setattr__(self, "state_dim", self.action_dim)
        if self.state_dim > self.action_dim:
            raise ValueError("state_dim cannot exceed action_dim")
    ...
    def inputs_spec(self, *, batch_size: int = 1) -> tuple[_model.Observation, _model.Actions]:
        ...
        with at.disable_typechecking():
            observation_spec = _model.Observation(
                ...
                future_state_dim_mask=(
                    jax.ShapeDtypeStruct([batch_size, self.action_dim], jnp.bool_)
                    if self.predict_future_states
                    else None
                ),
