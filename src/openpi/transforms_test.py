...
def test_extract_future_state_trajectory_and_padding():
    state_trajectory = np.arange(20, dtype=np.float32).reshape(5, 4)
    data = {
        "state": state_trajectory,
        "future_state_mask": np.array([True, True, False, False]),
    }
    data = _transforms.ExtractFutureStateTrajectory(action_horizon=4)(data)
    data = _transforms.PadStatesAndActions(model_action_dim=6)(data)

    np.testing.assert_array_equal(data["state"][:4], state_trajectory[0])
    np.testing.assert_array_equal(data["future_states"][:, :4], state_trajectory[1:])
    np.testing.assert_array_equal(data["future_state_mask"], [True, True, False, False])
    np.testing.assert_array_equal(data["future_state_dim_mask"], [True, True, True, True, False, False])


def test_create_future_state_mask_from_lerobot_padding():
    data = {
        "observation.state": np.zeros((4, 2), dtype=np.float32),
        "observation.state_is_pad": np.array([False, False, True, True]),
    }
    result = _transforms.CreateFutureStateMask(("observation.state",))(data)
    np.testing.assert_array_equal(result["future_state_mask"], [True, False, False])
