        ...
        def prepare_single_dataset(dataset_cfg: RLDSDataset):
            ...
            def chunk_actions(traj):
                ...
                if predict_future_states:
                    # State trajectory is q_t..q_{t+H}; the split into q_t and q_{t+1:t+H}
                    # happens after the robot-specific input transform.
                    state_chunk_indices = tf.broadcast_to(
                        tf.range(action_chunk_size + 1)[None],
                        [traj_len, action_chunk_size + 1],
                    ) + tf.broadcast_to(
                        tf.range(traj_len)[:, None],
                        [traj_len, action_chunk_size + 1],
                    )
                    valid_state_indices = state_chunk_indices < traj_len
                    state_chunk_indices = tf.minimum(state_chunk_indices, traj_len - 1)
                    traj["observation"]["joint_position"] = tf.gather(
                        traj["observation"]["joint_position"], state_chunk_indices
                    )
                    traj["observation"]["gripper_position"] = tf.gather(
                        traj["observation"]["gripper_position"], state_chunk_indices
                    )
                    traj["future_state_mask"] = valid_state_indices[:, 1:]
                return traj
